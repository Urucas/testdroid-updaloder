...
const api_endpoint = "http://appium.testdroid.com/upload"

function validSettings(settings) {
  if(!settings) return new Error("Undefined settings")
  if(!settings.user) return new Error("Undefined user")
  if(!settings.access_key) return new Error("Undefined password")
...
function upload(settings, cb, sync = false) {
  ...
  let logger = semafor()
  if(settings.verbose)
    logger.log("Uploding app to Testdroid, this may take a while!")
...
function syncPostRequest(settings) {
  ...
  try {
    res = syncRequest('POST', api_endpoint, req)
